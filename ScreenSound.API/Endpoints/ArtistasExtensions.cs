...
public static class ArtistasExtensions
{
    public static void AddEndPointsArtistas(this WebApplication app)
    {
        ...
        groupBuilder.MapPost("avaliacao", (HttpContext context,[FromServices] DAL<Artista> dalArtista, [FromBody] AvaliacaoArtistaRequest request, [FromServices] DAL<PessoaComAcesso> dalPessoa) =>
        {
            ...
            var email = context.User.Claims.FirstOrDefault(c=>c.Type== ClaimTypes.Email)?.Value?? throw new Exception("Usuário não autenticado");

            var pessoa = dalPessoa.RecuperarPor(p => p.Email!.Equals(email))?? throw new Exception("Pessoa não conectada!") ;
            ...
            return Results.Created($"/artistas/{artista.Id}", EntityToResponse(artista));
        });
